...
Use this converter to get the `nexus` file from the `phylip`
http://sequenceconversion.bugaco.com/converter/biology/sequences/fasta_to_nexus.php

Use the nexus file as input for **BEAUti** to get the `xml`file.
Use the `xml` as input of **BEAST** to get the `trees`.
Use the `tree` as input of **TreeAnnotator** to the the maximum credibility tree, which can be visualized in **FigTree**.
